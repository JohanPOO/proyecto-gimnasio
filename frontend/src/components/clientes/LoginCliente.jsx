  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:8000/api/inicio-sesion-cliente",
        {
          username,
          password,
        }
      ...
      <div className="bg-white shadow-lg rounded-md min-h-screen px-10 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="md:text-2xl sm:mx-auto sm:w-full sm:max-w-md">
          <h2 className="mt-6 text-center text-5xl font-extrabold text-gray-900">
            <p className="inline-block text-sky-600">Inicia Sesión</p> con tu
            cuenta
          </h2>
        </div>
          ...
          <div className="bg-white py-8 px-4 shadow-xl sm:rounded-lg sm:px-10">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                <button
                  type="submit"
                  className="w-full flex justify-center py-3 px-4 mb-4 border border-transparent rounded-md shadow-sm text-base uppercase font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  Iniciar Sesión
                </button>
                <a
